...
#%% CREATE INPUTS FOR XGBOOST/LAMBDAMART

# xTr = np.concatenate(query_embeddings_ns,passage_embeddings_ns,cosine_similarity_array,query_length,passage_length)
xTr_full = np.stack((cosine_similarity_array,query_length,passage_length, qids, relevance_scores), axis=1)
xTr, xTr_validation = train_test_split(xTr_full, test_size=0.3, stratify=xTr_full[:,-1])

#train vals
sorted_indices = np.argsort(xTr[:,-2])[::-1] #sort by qid
xTr = xTr[sorted_indices]
yTr = xTr[:,-1]
xTr = xTr[:, 0:3] #cosine sim, query len, passage len
# yTr = relevance_scores

#train_validation vals
sorted_indices = np.argsort(xTr_validation[:,-2])[::-1] #sort by qid
xTr_validation = xTr_validation[sorted_indices]
yTr_validation = xTr_validation[:,-1]
xTr_validation = xTr_validation[:, 0:3] #cosine sim, query len, passage len
...
feature_names = ['cosine_similarity', 'query_length', 'passage_length']
rank_metric = 'ndcg'
params = {'objective': 'rank:ndcg', 'eval_metric': 'ndcg', 'eta': 0.01, 'max_depth': 5}

dtrain = xgb.DMatrix(xTr, label=yTr, feature_names=feature_names)
dtrain_val = xgb.DMatrix(xTr_validation, label=yTr_validation, feature_names=feature_names)
dvalidation = xgb.DMatrix(xTe, label=yTe, feature_names=feature_names)

unique_qid_list, qid_counts = np.unique(xTr[:,-2], return_counts=True)
qid_counts = qid_counts.tolist()
unique_qid_list_trainval, qid_counts_trainval = np.unique(xTr_validation[:,-2], return_counts=True)
qid_counts_trainval = qid_counts_trainval.tolist()
unique_qid_list_validation, qid_counts_validation = np.unique(qids_validation, return_counts=True)
qid_counts_validation = qid_counts_validation.tolist()

dtrain.set_group(qid_counts)
dtrain_val.set_group(qid_counts_trainval)
dvalidation.set_group(qid_counts_validation)

model = xgb.train(params, dtrain, num_boost_round=100, evals=[(dtrain_val, 'validation')])

# Save the trained model just in case
model.save_model(fname='model.json')
# Load the trained model 
bst = xgb.Booster(model_file='model.json')

# y_pred = model.predict(dvalidation)
y_pred = np.expand_dims(bst.predict(dvalidation, ntree_limit=bst.best_ntree_limit), axis=1)
# y_pred = np.expand_dims(model.predict(dvalidation), axis=1)

# LM_validation_queries_np = np.concatenate((validation_queries_np, y_pred))
LM_validation_queries_np = np.hstack((validation_queries_np, y_pred))
...
def rank_array_by_predicted_relevance(array):
    ...
    for qid in unique_qids:                                         
        qid_stack = sorted_array[sorted_array[:,0] == qid][:100,:] 
...
sorted_LM_validation_queries_np = rank_array_by_predicted_relevance(LM_validation_queries_np)
np.save('sorted_LM_validation_queries_np.npy', sorted_LM_validation_queries_np)
#UNCOMMENT ONCE DONE
sorted_LM_validation_queries_np = np.load('sorted_LM_validation_queries_np.npy', allow_pickle=True)
...
def compute_ndcg(predictions, k=2):
    ...
    for query_id in np.unique(sorted_predictions[:, 0]): # iterate through the queries
...
LM_MeanAP = mean_avg_precision(sorted_LM_validation_queries_np)
print("LambdaMART Mean Average Precision: ", LM_MeanAP)
sorted_ndcg_LM = compute_ndcg(sorted_LM_validation_queries_np, kay)
print("LambdaMART average NDCG: ", sorted_ndcg_LM)
...
dTest = xgb.DMatrix(xTTest, feature_names=feature_names)
unique_qid_list_allpass, qid_counts_allpass = np.unique(qids_allpass, return_counts=True)
qid_counts_allpass = qid_counts_allpass.tolist()
dTest.set_group(qid_counts_allpass)

y_pred_Test = np.expand_dims(bst.predict(dTest, ntree_limit=bst.best_ntree_limit), axis=1) #CHANGED BST TO MODEL BELOW


LM_Test_queries_np = np.hstack((all_passages_np, y_pred_Test))
sorted_LM_Test_queries_np = rank_array_by_predicted_relevance(LM_Test_queries_np)
np.save('sorted_LM_Test_queries_np.npy', sorted_LM_Test_queries_np)
sorted_LM_Test_queries_np = np.load('sorted_LM_Test_queries_np.npy', allow_pickle=True)
...
for i in range(test_queries_np.shape[0]):
    ...
    pids_list_for_qid = sorted_LM_Test_queries_np[sorted_LM_Test_queries_np[:,0] == qid]
    for j in range(pids_list_for_qid.shape[0]):
        qid = str(qid)
        pid = str(pids_list_for_qid[j,1])
        score = str(pids_list_for_qid[j,-1])
        f.write(qid + "," + "A2" + "," + pid + "," + str(rank) + "," + score + "," + "LM" + "\n")
        rank += 1
f.close()
